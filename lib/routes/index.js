...
function Routes() {
  ...
  function filterKeys(object, type, val) {
    ...
    for (var key in object) {
      if (type=='ends_with') {
        if (key.indexOf(val, key.length - val.length) !== -1) {
  ...
  app.post('/mapred', function(req, res, next) {
    ...
    _.forOwn(objects, function(object, key) {
      var mappingObject = { key: key, values: [{ metadata: {index: object.indexes}, indexes: object.indexes, data: JSON.stringify(object.data) }] };
